...
class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        player = this.physics.add.image(10, 216, 'Player').setScale(0.15).setCollideWorldBounds(true).setDepth(5)
    ...
    TREE() {
        event = this.time.addEvent({
            ...
            callback: function () {
                let tree = this.physics.add.image(player.x + 60, player.y - 80,'tree')
                tree.setScale(1).setSize(0.1)
                trees.add(tree)
            },
            callbackScope: this,
            loop: false,
